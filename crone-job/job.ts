...
class job {
  ...
  public checkStock() {
    const timeInterval = "*/15 * * * * *"; // Every 15 seconds
    // const timeInterval = "0 0 * * *"; // 12 am Everyday
    schedule.scheduleJob(timeInterval, async function () {
      ...
      try {
        ...
        const inStockAliExpress = await page.evaluate(() => {
          const inStock = document.querySelector(
            ".product-quantity-tip span span"
          );
          const inStockQuantity =
            inStock && inStock.innerHTML
